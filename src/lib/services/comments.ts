...
export const get = async (id: number): Promise<PostCommentType | null> => {
    const { data: postData, error } = await supabase.from('comments').select('*').eq('id', id).single();
    if (error) {
...
export const getPostComments = async (post_id: number): Promise<PostCommentType[] | null> => {
    const { data: postData, error } = await supabase.from('comments').select('*').eq('post_id', post_id);
    if (error) {
        console.error('Error fetching post:', error.message);
    } else {
        if (!postData) {
...
export const create = async (comment: PostCommentType): Promise<PostCommentType | null> => {
    const { data: commentData, error } = await supabase.from('comments').insert(comment).select().single();
...
export const update = async (comment: PostCommentType): Promise<PostCommentType | null> => {
    const { data: postData, error } = await supabase.from('comments').update(comment).eq('id', comment.id).single();
    if (error) {
